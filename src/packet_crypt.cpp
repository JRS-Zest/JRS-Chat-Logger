#include "packet_crypt.h"

// C# PacketCrypt.cs から完全移植したテーブル
const uint8_t PacketCrypt::AucDataTable[448] = {
    
};

const uint8_t PacketCrypt::XorKeys[213] = {
    
};

int PacketCrypt::GenerateScenarioDecodeKey(int seed) {
    if (seed == -1) return 0;
    return seed == 1 ? 2 : 1;
}

std::vector<uint8_t> PacketCrypt::DecodeScenarioBuffer(const uint8_t* data, size_t len, int decodeKey) {
    if (!data || len == 0) return {};

    int tableOffset = (((decodeKey * 9) << 3) - decodeKey) & 0xFF;
    int encCounter = 0;
    std::vector<uint8_t> result(len);

    for (size_t i = 0; i < len; i++) {
        int idx = encCounter + tableOffset;
        uint8_t a = AucDataTable[idx];
        uint8_t x = XorKeys[idx];
        uint8_t endata = static_cast<uint8_t>((a + x) & 0xFF);
        result[i] = data[i] ^ endata;
        encCounter++;
        if (encCounter >= 0x47) encCounter = 0;
    }

    return result;
}
